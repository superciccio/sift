...
-spec min_length(integer(), binary()) -> fun((list(ABH)) -> {ok, list(ABH)} |
    {error, binary()}).
min_length(N, Msg) ->
...
-spec max_length(integer(), binary()) -> fun((list(ABM)) -> {ok, list(ABM)} |
    {error, binary()}).
max_length(N, Msg) ->
    fun(Value) -> case erlang:length(Value) =< N of
            true ->
...
-spec non_empty(binary()) -> fun((list(ABR)) -> {ok, list(ABR)} |
    {error, binary()}).
non_empty(Msg) ->
    min_length(1, Msg).
