...
-spec be_ok({ok, DOT} | {error, any()}) -> DOT.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DOY}) -> DOY.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DPB)) -> DPB.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
